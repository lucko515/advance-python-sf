...
def delegating_generator():
    """Delegates to subgenerator using yield from"""
    print("  Starting delegating generator")
    yield from subgenerator()  # Delegate to subgenerator
    print("  Finished with subgenerator, yielding more values")
    yield "D"
    yield "E"
...
def nested_without_yield_from():
    """Does the same thing without yield from"""
    print("  Starting nested generator")
...
def subgen_with_return():
    ...
    yield "X"
    yield "Y"
    return "Subgenerator result"  # This return value is passed to the delegating generator

def delegating_with_return():
    """Shows how to capture the return value from a subgenerator"""
...
# Properly closing generators with close()
print("\nClosing generators with close():")

def generator_with_cleanup():
    """A generator with cleanup code"""
    try:
        print("  Generator started")
        yield "First"
        ...
        yield "Third"
        print("  Generator completed naturally")
    finally:
        print("  Generator cleanup (close called)")

print("\nUsing close() to clean up a generator:")
gen_clean = generator_with_cleanup()
print(f"  Got: {next(gen_clean)}")  # "First"
print("  Calling close() on the generator...")
gen_clean.close()
